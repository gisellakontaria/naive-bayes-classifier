...
void NaiveBayesApp::keyDown(ci::app::KeyEvent event) {
  switch (event.getCode()) {
    case ci::app::KeyEvent::KEY_RETURN:
      image_to_classify_.image_vector_ = sketchpad_.vec_;

      // ask classifier to classify the image that's currently drawn on the
      // sketchpad and update current_prediction_
      image_to_classify_.image_vector_ = sketchpad_.vec_;
      current_prediction_ = classifier_.ClassifyImage(image_to_classify_);
      break;
